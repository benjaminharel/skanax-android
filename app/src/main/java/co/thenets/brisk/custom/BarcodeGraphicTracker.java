...
public class BarcodeGraphicTracker extends Tracker<Barcode>
{
    ...
    private Barcode mLastItemBarcode = null;
    ...
    {
            mGraphic.setId(id);
            if(item.rawValue.length() == 13 )
            {
                EventsManager.getInstance().post(new BarcodeDetectedEvent(item.rawValue));
                EventsManager.getInstance().post(new ItemAddedMyInventoryEvent());
            }
        Log.i("new Item add: ", item.rawValue);
    }
